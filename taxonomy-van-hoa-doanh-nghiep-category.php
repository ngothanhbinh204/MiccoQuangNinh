                ...
                <div class="block-wrap xl:rem:max-w-[1110px] 2xl:rem:max-w-[1020px]">
                    ...
                    <div class="grid gap-6 sm:grid-cols-2">
                        <?php foreach($items as $index => $item): ?>
                            <div class="item overflow-hidden rounded-6 p-6 bg-secondary-bg y-start" data-fancybox="" href="#culture-modal-<?php echo $index + 1; ?>">
                                ...
                                <div class="text-wrap ml-6">
                                    ...
                                    <div class="desc body-1"><?php echo wp_trim_words($item['description'], 15, '...'); ?></div>
                                ...
                                <div class="popup-modal culture-modal hidden" id="culture-modal-<?php echo $index + 1; ?>">
                                    <div class="modal-wrap relative z-50 gap-8 lg:gap-16 md:x-start">
                                        ...
                                        <div class="txt col-left">
                                            <div class="icon flex-start w-[64px] min-w-[64px] h-[64px] lg:w-16 lg:min-w-16 lg:h-16"><?php echo get_image_attrachment($item['icon'], 'image'); ?></div>
...
<?php if(get_field('trip',$term)): ?>
    <?php
    $trip = get_field('trip',$term);
    $title = $trip['title'];
    $items = $trip['items'];
    ?>
...
<?php if(get_field('activities',$term)): ?>
    <?php 
    $activities = get_field('activities',$term);
    $title = $activities['title'];
    $select_activities = $activities['select_activities'];
    ?>
            ...
            <div class="row"> 
                <?php if($select_activities): ?>
                    ...
                    <div class="col w-full lg:w-4/12">
                        <?php foreach($select_activities as $index => $item): ?>
                            <?php if($index > 0): ?>
                                <div class="side-figure overflow-hidden group">
                                    <div class="txt">
                                        <time class="mb-2 text-grey-950"><?php echo get_the_date('d.m.Y', $item->ID); ?></time>
                                        <h3 class="headline"><a class="group-hover:text-primary-1 heading-4 line-clamp-2" href="<?php echo get_permalink($item->ID); ?>"><?php echo get_the_title($item->ID); ?></a></h3>
                                    </div>
...
<?php if(get_field('programme',$term)): ?>
    <?php
...
<?php if(get_field('members',$term)): ?>
    <?php
    $member = get_field('members',$term);
    $title = $member['title'];
